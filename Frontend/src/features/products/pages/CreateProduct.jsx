...
export const CreateProduct = () => {
  ...
  const handleSizeToggle = (variantId, size) => {
    setVariants((prev) =>
      prev.map((variant) => {
  ...
  const handleProductSizeToggle = (size) => {
  ...
  const handleProductSizeStockChange = (size, value) => {
    setSizeStocks((prev) => ({
      ...prev,
      [size]: value,
    }));
  ...
  const handleVariantImages = (id, files) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const preparedProductSizes = {};

      selectedSizes.forEach((size) => {
        preparedProductSizes[size] =
          Number(sizeStocks[size]) || 0;
      });

      payload.append(
        "sizes",
        JSON.stringify(preparedProductSizes)
      );
      ...
      payload.append(
        "variants",
        JSON.stringify(preparedVariants)
      );

      const variantImageCounts = variants.map(
            ...
            <section className="lg:col-span-5 flex flex-col gap-0 lg:sticky lg:top-[68px]">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="flex items-center bg-white border border-neutral-200 rounded-md overflow-hidden transition-all duration-150 focus-within:ring-2 focus-within:border-neutral-800 focus-within:ring-neutral-100">
                    <input
                      id="product-stock"
                      name="stock"
                      type="number"
                      ...
                      onChange={handleInputChange}
                      className="w-full px-3.5 py-[10px] text-[14px] bg-transparent text-neutral-900 placeholder:text-neutral-300 focus:outline-none tabular-nums"
                    />
                  </div>
                ...
                {variants.length === 0 && (
                  <div className="pt-6 border-t border-neutral-200">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <p className="mt-1 text-[11px] leading-5 text-neutral-400">
                          Optional. Add sizes if this product is available in different sizes.
                        </p>
                      </div>
                      ...
                      {SIZE_OPTIONS.map((size) => {
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              handleProductSizeToggle(size)
                            }
                            className={`min-w-[42px] rounded-md border px-3 py-2 text-[11px] font-medium transition-all ${isSelected
                    ...
                    {/* Stock by selected size */}
                    {selectedSizes.length > 0 && (
                      <div className="mt-5">
                        <label className="mb-2 block text-[10px] font-semibold tracking-[0.12em] uppercase text-neutral-400">
                          Stock by size
                        </label>
                        ...
                        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                          {selectedSizes.map((size) => (
                            <div key={size}>
                              <span className="mb-1.5 block text-[10px] text-neutral-400">
                                {size}
                              </span>
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  handleProductSizeStockChange(
                                    size,
                                    e.target.value
                                  )
                                }
                                placeholder="0"
                                className="w-full rounded-md border border-neutral-200 bg-white px-3 py-2.5 text-[13px] text-neutral-900 placeholder:text-neutral-300 outline-none transition-all focus:border-neutral-800 focus:ring-2 focus:ring-neutral-100"
                              />
                            </div>
                          ))}
                        </div>
                ...
                <div className="pt-6 border-t border-neutral-200">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-1 text-[11px] leading-5 text-neutral-400">
                        Add color, size, stock, and images for each variant.
                      </p>
                    </div>
                    ...
                    <button
                      ...
                      className="shrink-0 rounded-md border border-neutral-200 bg-white px-3.5 py-2 text-[10px] font-semibold tracking-[0.12em] uppercase text-neutral-700 transition-all hover:border-neutral-800 hover:text-neutral-900"
                    >
                      + Add Variant
                    </button>
                  </div>
                  ...
                  {/* Empty state */}
                  {variants.length === 0 && (
                    <div className="mt-5 rounded-md border border-dashed border-neutral-200 bg-neutral-50/50 px-5 py-8 text-center">
                      <p className="text-[12px] text-neutral-400">
                        No variants added
                      </p>

                      <p className="mt-1 text-[10px] text-neutral-300">
                        Add variants if this product has different colors or sizes.
                      </p>
                    </div>
                  )}
                        ...
                        >
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <button
                              ...
                              className="text-[10px] tracking-[0.1em] uppercase text-neutral-400 transition-colors hover:text-rose-500"
                            >
                              Remove
                            </button>
                          </div>
                          ...
                          <div className="mt-5">
                            <div className="flex items-center justify-between">
                              <label className="text-[10px] font-semibold tracking-[0.12em] uppercase text-neutral-400">
                                Sizes
                              </label>
                            ...
                            <p className="mt-2 text-[10px] text-neutral-300">
                              Leave empty if this product does not have sizes.
                            </p>
                          </div>
                          ...
                          <div className="mt-5">
                            ...
                            <div className="mt-3 flex flex-wrap gap-2">
                              ...
                              {variant.images.map((image) => (
                                <div
                                  key={image.id}
                                  className="group relative h-20 w-16 overflow-hidden rounded-sm bg-neutral-100"
                                >
                                  <img
                                  ...
                                  <button
                                    ...
                                    className="absolute right-1 top-1 flex h-5 w-5 items-center justify-center rounded-full bg-white/90 text-[12px] text-neutral-600 opacity-0 shadow-sm transition-opacity group-hover:opacity-100"
                                  >
                                    ×
                                  </button>
                                </div>
                              ...
                              {/* Add image */}
                              <label className="flex h-20 w-16 cursor-pointer flex-col items-center justify-center rounded-sm border border-dashed border-neutral-200 bg-neutral-50 text-neutral-400 transition-all hover:border-neutral-400 hover:text-neutral-700">
                                <span className="text-lg leading-none">
                                  +
                                </span>

                                <span className="mt-1 text-[8px] font-semibold tracking-[0.08em] uppercase">
                                  Add
                                </span>
